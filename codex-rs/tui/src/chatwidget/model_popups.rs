...
impl ChatWidget {
    ...
    fn model_menu_warning_line(&self) -> Option<Line<'static>> {
        ...
        let warning = format!(
            "Warning: the provider base URL is overridden to {base_url}. Selecting models may not be supported or work properly."
        );
    ...
    pub(crate) fn open_all_models_popup(&mut self, presets: Vec<ModelPreset>) {
        ...
        let header = self.model_menu_header(
            ...
            "Access legacy models by running redapto -m <model_name> or in your config.toml",
        );
        self.bottom_pane.show_selection_view(SelectionViewParams {
