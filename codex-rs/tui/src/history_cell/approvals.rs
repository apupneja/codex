        ...
        Approved => match subject {
            ...
            ApprovalDecisionSubject::NetworkAccess { target } => (
                ...
                vec![
                    actor.subject().into(),
                    "approved".bold(),
                    " Redapto network access to ".into(),
        ...
        ApprovedExecpolicyAmendment {
            ...
            (
                ...
                vec![
                    actor.subject().into(),
                    "approved".bold(),
                    " Redapto to always run commands that start with ".into(),
                    snippet,
                ],
            )
        }
        ...
        TimedOut => match subject {
            ...
            ApprovalDecisionSubject::NetworkAccess { target } => (
                ...
                vec![
                    "Review ".into(),
                    "timed out".bold(),
                    " before Redapto could access ".into(),
                    Span::from(target).dim(),
                ],
            ),
        },
        Abort => match subject {
            ...
            ApprovalDecisionSubject::NetworkAccess { target } => (
                ...
                vec![
                    actor.subject().into(),
                    "canceled".bold(),
                    " the request for Redapto network access to ".into(),
                    Span::from(target).dim(),
                ],
            ),
        },
...
pub fn new_guardian_timed_out_patch_request(files: Vec<String>) -> Box<dyn HistoryCell> {
    let mut summary = vec![
        "Review ".into(),
        "timed out".bold(),
        " before Redapto could apply ".into(),
    ];
    if files.len() == 1 {
        summary.push("a patch touching ".into());
        summary.push(Span::from(files[0].clone()).dim());
