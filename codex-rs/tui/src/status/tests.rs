...
#[tokio::test]
async fn status_snapshot_shows_hosted_plan_without_email() {
    let temp_home = TempDir::new().expect("temp home");
...
async fn status_snapshot_truncates_halfwidth_kana_in_narrow_terminal() {
    ...
    let account = StatusAccountDisplay::Provider {
...
async fn status_snapshot_uses_default_reasoning_when_config_empty() {
    ...
    let remote_connection = RemoteConnectionStatus {
        address: "unix:///tmp/redapto-home/app-server-control/app-server-control.sock".to_string(),
