...
#[command(about = "Tail Redapto logs from the dedicated logs SQLite DB with simple filters")]
struct Args {
    /// Path to the Redapto home. Defaults to $REDAPTO_HOME or ~/.redapto.
    #[arg(long, env = "REDAPTO_HOME")]
...
mod tests {
    ...
    /// Explicit database selection must not parse an overridden Redapto home.
    #[tokio::test]
    async fn direct_db_skips_codex_home_config() {
        let codex_home = tempfile::tempdir().expect("create Redapto home");
