...
pub(crate) struct SandboxSetupCommand {
    ...
    /// Windows user that will run Redapto after managed deployment.
    #[arg(
        long = "user",
        value_name = "USER",
    ...
    /// Use the current Windows user as the Redapto user.
    #[arg(
        long = "current-user",
    ...
    /// Data home for the Redapto user. Required with --user.
...
impl SandboxSetupCommand {
    fn setup_level(&self) -> anyhow::Result<SandboxSetupLevel> {
        if self.elevated_sandbox_level {
            ...
            anyhow::bail!("`redapto sandbox setup` currently requires --elevated");
        }
    }
}
...
async fn run_elevated(
    ...
    let config = ConfigBuilder::default()
        ...
        .build()
        .await
        .context("failed to load target user's Redapto config for sandbox provisioning")?;
