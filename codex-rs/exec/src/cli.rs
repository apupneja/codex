...
#[command(
    version,
    override_usage = "redapto exec [OPTIONS] [PROMPT]\n       redapto exec [OPTIONS] <COMMAND> [ARGS]"
)]
pub struct Cli {
    /// Process-only PSP routing selected by the parent Redapto CLI.
    #[clap(skip)]
    pub psp: bool,
    ...
    /// Allow running Redapto outside a Git repository.
    ...
    /// Do not load `$REDAPTO_HOME/config.toml`; credentials still use the Redapto home.
...
struct ResumeArgsRaw {
    ...
    // so "redapto resume --last <prompt>" treats the positional as a prompt, not a session id.
