...
fn usage_limit_reached_error_formats_plus_plan() {
    ...
    assert_eq!(
        err.to_string(),
        "You've hit your usage limit. Contact your provider to increase limits or try again later."
    );
}
...
fn usage_limit_reached_error_formats_pro_plan_with_reset() {
    ...
    with_now_override(base, move || {
        ...
        let expected = format!(
            "You've hit your usage limit. Contact your provider to increase limits or try again at {expected_time}."
        );
        assert_eq!(err.to_string(), expected);
    });
}
...
fn usage_limit_reached_with_promo_message() {
    ...
    with_now_override(base, move || {
        ...
        let err = UsageLimitReachedError {
            ...
            resets_at: Some(resets_at),
            rate_limits: Some(Box::new(rate_limit_snapshot())),
            promo_message: Some("Contact your provider to activate <PLAN>".to_string()),
            rate_limit_reached_type: None,
        };
        let expected = format!(
            "You've hit your usage limit. Contact your provider to activate <PLAN>, or try again at {expected_time}."
        );
        assert_eq!(err.to_string(), expected);
    });
}
