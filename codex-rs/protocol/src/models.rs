...
fn local_media_error_placeholder(
    ...
    ContentItem::InputText {
        text: format!("Redapto could not read the local {media_name} at `{path}`: {error}"),
    }
}
...
fn unsupported_image_error_placeholder(path: &std::path::Path, mime: &str) -> ContentItem {
    ContentItem::InputText {
        text: format!(
            "Redapto cannot attach image at `{}`: unsupported image `{}`.",
            path.display(),
            mime
        ),
    }
...
fn unsupported_audio_error_placeholder(path: &std::path::Path) -> ContentItem {
    ContentItem::InputText {
        text: format!(
            "Redapto cannot attach audio at `{}`: unsupported audio format; use wav, mp3, m4a, webm, or ogg.",
            path.display()
        ),
    }
}
...
- ["a", "b", "c"]"#
    ...
    fn replaces_unreadable_local_audio_with_placeholder() {
        ...
        assert!(
            text.starts_with("Redapto could not read the local audio at `missing.wav`: "),
...
<svg xmlns="http://www.w3.org/2000/svg" width="1" height="1"></svg>"#,
        ...
        match item {
            ResponseInputItem::Message { content, .. } => {
                assert_eq!(content.len(), 1);
                let expected = format!(
                    "Redapto cannot attach image at `{}`: unsupported image `image/svg+xml`.",
                    svg_path.display()
                );
                match &content[0] {
