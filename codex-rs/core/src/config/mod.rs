...
fn resolve_sqlite_home_env(resolved_cwd: &Path) -> Option<AbsolutePathBuf> {
    let raw = std::env::var(codex_state::SQLITE_HOME_ENV)
        .ok()
        .or_else(|| std::env::var(codex_state::LEGACY_SQLITE_HOME_ENV).ok())?;
...
impl Permissions {
    ...
    /// selection. Internal Redapto-only writable roots are intentionally excluded.
...
// when Redapto is still responsible for the network policy. `Disabled` means no
...
pub struct Config {
    ...
    /// Optional external notifier command. When set, Redapto will spawn this
    /// program after each completed *turn* (i.e. when the agent finishes
    /// processing a user submission). The value must be the full command
    /// broken into argv tokens **without** the trailing JSON argument - Redapto
    /// appends one extra argument containing a JSON payload describing the
    /// event.
    ///
    /// Example `~/.redapto/config.toml` snippet:
    ///
    /// ```toml
    /// notify = ["notify-send", "Redapto"]
    /// ```
    ///
    /// which will be invoked as:
    ///
    /// ```shell
    /// notify-send Redapto '{"type":"agent-turn-complete","turn-id":"12345"}'
    /// ```
    ///
    ...
    /// file (default): Use a file in the Redapto home directory.
    /// keyring: Use an OS-specific keyring service.
    /// auto: Use the OS-specific keyring service if available, otherwise use a file.
    pub cli_auth_credentials_store_mode: AuthCredentialsStoreMode,

    /// Definition for MCP servers that Redapto can reach out to for tool calls.
    ...
    /// keyring: Use an OS-specific keyring service.
    ///          Credentials stored in the keyring will only be readable by Redapto unless the user explicitly grants access via OS-level keyring access.
    ///          https://github.com/openai/codex/blob/main/codex-rs/rmcp-client/src/oauth.rs#L2
    /// file: REDAPTO_HOME/.credentials.json
    ///       This file will be readable to Redapto and other applications running as the same user.
    /// auto (default): keyring if available, otherwise file.
    pub mcp_oauth_credentials_store_mode: OAuthCredentialsStoreMode,

    /// Optional fixed port to use for the local HTTP callback server used during MCP OAuth authorization.
    ///
    /// When unset, Redapto will bind to an ephemeral port chosen by the OS.
    pub mcp_oauth_callback_port: Option<u16>,

    /// Optional redirect URI to use during MCP OAuth authorization.
    ///
    ...
    /// Directory containing all Redapto state (defaults to `~/.redapto` but can be
    /// overridden by the `REDAPTO_HOME` environment variable).
    pub codex_home: AbsolutePathBuf,

    /// Resolved configuration shared by all Redapto SQLite databases.
    pub sqlite: codex_state::SqliteConfig,

    /// Directory where Redapto writes log files (defaults to `$REDAPTO_HOME/log`).
    pub log_dir: PathBuf,

    /// Directory where Redapto writes effective session config lock files.
    pub config_lock_export_dir: Option<AbsolutePathBuf>,

    /// Whether config lock replay ignores Redapto version drift between the
    ...
    /// Settings that govern if and what will be written to `~/.redapto/history.jsonl`.
    pub history: History,
    ...
    /// Path to the current Redapto executable. This cannot be set in the config
    ...
    /// Base URL for requests to provider service (as opposed to the provider API).
    pub chatgpt_base_url: String,

    /// Whether Redapto-owned clients should respect host system proxy settings.
    pub respect_system_proxy: bool,

    /// Process-only provider service routing selection supplied when Redapto is launched.
    pub psp: bool,
    ...
    /// When set, restricts hosted-provider authentication to one or more workspace identifiers.
    pub forced_chatgpt_workspace_id: Option<Vec<String>>,

    /// When set, restricts the authentication mechanism users may use.
    ...
    /// When `true`, checks for Redapto updates on startup and surfaces update prompts.
    /// Set to `false` only if your Redapto updates are centrally managed.
    /// Defaults to `true`.
    ...
    /// When `false`, disables analytics across Redapto product surfaces in this machine.
    /// Voluntarily left as Optional because the default value might depend on the client.
    pub analytics_enabled: Option<bool>,

    /// When `false`, disables feedback collection across Redapto product surfaces.
    /// Defaults to `true`.
...
impl Config {
    ...
    /// Load a default configuration for a specific Redapto home without reading
...
/// Patch `REDAPTO_HOME/config.toml` project state to set trust level.
/// Use with caution.
...
/// Returns the path to the Redapto configuration directory, which can be
/// specified by the `REDAPTO_HOME` environment variable. If not set, defaults to
/// `~/.redapto`.
///
/// - If `REDAPTO_HOME` is set, the value must exist and be a directory. The
///   value will be canonicalized and this function will Err otherwise.
/// - If `REDAPTO_HOME` is not set, this function does not verify that the
///   directory exists.
pub fn find_codex_home() -> std::io::Result<AbsolutePathBuf> {
    codex_utils_home_dir::find_codex_home()
}

/// Returns the path to the folder where Redapto logs are stored. Does not verify
