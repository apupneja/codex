...
fn missing_filesystem_entries_warning(profile_name: &str) -> String {
    format!(
        "Permissions profile `{profile_name}` does not define any recognized filesystem entries for this version of Redapto. Filesystem access will remain restricted. Upgrade Redapto if this profile expects filesystem permissions."
    )
}
...
fn maybe_push_unknown_special_path_warning(
    ...
    push_warning(
        ...
        match subpath.as_deref() {
            Some(subpath) => format!(
                "Configured filesystem path `{path}` with nested entry `{subpath}` is not recognized by this version of Redapto and will be ignored. Upgrade Redapto if this path is required."
            ),
            None => format!(
                "Configured filesystem path `{path}` is not recognized by this version of Redapto and will be ignored. Upgrade Redapto if this path is required."
            ),
        },
    );
}
