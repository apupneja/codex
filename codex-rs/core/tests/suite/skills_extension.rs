...
async fn production_turn_fairly_shortens_extension_catalog_descriptions() -> Result<()> {
    ...
    assert_eq!(
        warning.message,
        "Skill descriptions were shortened to fit the skills context budget. Redapto can still see every skill, but some descriptions are shorter. Disable unused skills or plugins to leave more room for the rest."
    );
