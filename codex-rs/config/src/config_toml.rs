//! Schema-heavy configuration TOML types used by Redapto.
...
/// Backward-compatible shape for hosted-provider workspace authentication restrictions in config.toml.
...
/// Base config deserialized from ~/.redapto/config.toml.
...
pub struct ConfigToml {
    ...
    /// sanctioned by Redapto will likely degrade model performance.
    ...
    /// file: Use a file in the Redapto home directory.
    /// auto (default): Use the OS-specific keyring service if available, otherwise use a file.
    #[serde(default)]
    pub mcp_oauth_credentials_store: Option<OAuthCredentialsStoreMode>,

    /// Optional fixed port for the local HTTP callback server used during MCP OAuth authorization.
    ...
    /// Directory where Redapto stores the SQLite state DB.
    /// Defaults to `$REDAPTO_SQLITE_HOME` when set. Otherwise uses `$REDAPTO_HOME`.
    pub sqlite_home: Option<AbsolutePathBuf>,

    /// Directory where Redapto writes log files. Setting this value explicitly
    /// also enables the TUI text log in this directory.
    /// Defaults to `$REDAPTO_HOME/log`.
    ...
    /// Optional product SKU forwarded on host-owned Redapto Apps MCP requests.
...
pub struct DebugConfigLockToml {
    ...
    /// Allow replaying a lock generated by a different Redapto version.
